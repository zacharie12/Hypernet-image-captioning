...
class HyperNet(pl.LightningModule):
    def __init__(self, feature_size, embed_size, hidden_size, vocab_size, vocab, num_layers=1, lr=1e-6, mixup=False, alpha=0.1):
        super().__init__()
        ...
        self.teacher_forcing_proba = 0.0001
        self.beam_size = 3
    ...
    def training_step(self, train_batch, batch_idx):
        ...
        if self.mixup:
            ...
            label1, label2= torch.tensor([self.labels[style1]]).to(device), torch.tensor([self.labels[style2]]).to(device)
            ...
            if no_style == "factual":
                target = torch.tensor([0.0, coeff, 1-coeff])
            elif no_style == "humour":
                target = torch.tensor([coeff, 0.0, 1-coeff])
            else:
                target = torch.tensor([coeff,1-coeff, 0.0])
            loss_mixup = F.mse_loss(style_pred, target)  


        loss =  F.cross_entropy(caps_pred.view(-1, self.hparams['vocab_size']), caps.view(-1).long(), ignore_index=self.vocab.w2i['<pad>']) 
        if self.mixup:
            loss =  self.alpha * loss + (1-self.alpha) * loss_mixup

        bleu1, bleu2, bleu3, bleu4, bleu, meteor, rouge = metric_score(caps, caps_pred, self.vocab, self.metrics)
...
if __name__ == "__main__":
    ...
    save_path = "/cortex/users/cohenza4/checkpoint_gru/HN/pretrain/humour/"
    ...
    train_loader = DataLoader(train_data, batch_size=64, num_workers=24,
                            ...
                            lambda x: flickr_collate_style(x, 'romantic')
